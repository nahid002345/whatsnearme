package com.diordna.whats.near.me.header;
...
public class WhatsappHeaderBehavior extends CoordinatorLayout.Behavior<HeaderView> {
    ...
    public boolean onDependentViewChanged(CoordinatorLayout parent, HeaderView child, View dependency) {
        ...
        if (isHide && percentage < 0.9) {
            child.setVisibility(View.VISIBLE);
            mToolbar.setBackgroundColor(dependency.getResources().getColor(R.color.transparent));
            isHide = false;
        } else if (!isHide && percentage >= 0.9) {
            mToolbar.setBackgroundColor(dependency.getResources().getColor(R.color.primaryColor));
